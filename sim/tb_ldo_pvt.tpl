...
* ⛔ PORT ORDER IS THE SYM EDGE ORDER -- left, right, top, bottom -- NOT the order in
* the cell spec's `ports` list, and NOT alphabetical. ldo_vref is
*   vref_bg vref vref_ov vss vref_pg
* because vref_ov sits on the top edge. Writing it in the `ports` order instead tied
* the new tap to vss, and the whole block stopped regulating -- vout came out at
* 0.97 mV and the 243-corner sweep then reported 61 deg of phase margin on a dead
* circuit, which looked like a 15 deg IMPROVEMENT. Read the .subckt line; do not
* reconstruct it.
x_vref vref_bg vref vref_ov vss vref_pg ldo_vref
...
x_boost vout vfb vref_ov ibias eout vin vss ldo_boost
...
XCc   eout vout cap_cmomf w=40u l=40u mmin=1 mmax=4
XCout vout vss  cap_cmomf w=125u l=125u mmin=1 mmax=4
.control
  op
  print v(vout)
  print -i(Vin)
